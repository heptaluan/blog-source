...
至此，我们可以发现问题的根源所在，那就是 `TCP` 连接，那么有人就会问了，我们不使用 `TCP` 不就可以了吗？答案是肯定的，所以这也就引出了我们今天的主角，`HTTP/3` 和 `QUIC`
...
> 关于 `UDP` 与 `TCP` 的区别可见之前整理过的 [TCP 和 UDP 的比较](http://localhost:4000/2020/09/01/HTTP/10/#UDP)
...
`TCP` 拥塞控制由 `4` 个核心算法组成，即 **慢启动、拥塞避免、快速重传和快速恢复**，理解了这 `4` 个算法，对 `TCP` 的拥塞控制也就有了大概了解
