...
本章我们继续来看 `Node.js` 当中的 `http` 模块，其他部分可以参考以下目录
...
它还是一个 `EventEmitter`，包含 `response`、`socket`、`upgrade`、`continue` 等事件
...
在处理 `http` 请求的场景下，当新的请求到来时，我们通过调用 `parsers.alloc()` 方法来获取 `HTTPParser` 对象，从而解析 `http` 请求
