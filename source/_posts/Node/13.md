...
在 [上一章](http://localhost:4000/2019/10/19/Node/12/) 当中，我们简单的介绍了中间件的基本概念，以及洋葱模型，在最后我们也手动实现了一个简单的 `compose()` 方法，所以本章当中我们就主要手动的来实现一个最基本的 `koa.js` 框架以及 `koa.js` 当中一些比较常用的中间件的简单实现，比如 `koa-logger` 和 `koa-static` 等，关于其他部分可以参考以下目录
...
发现是可以正常使用的，但是这里面有一个问题，就是我们在处理中间件队列的时候，底层使用的是回调嵌套去处理的，但是中间件越多，回调嵌套越深，代码的可读性和可扩展性就很差，所以我们就可以考虑将我们的 `handleRequest` 方法调整为 `async/await` 方式
...
这一个中间件，在官方当中比较有代表性的是 `koa-ejs` 中间件，它实现了代理上下文（`context`），即把渲染的方法挂载在 `koa` 实例 `app` 的 `app.context` 属性中，所以这里我们就简单的实现一个模版渲染中间件来模仿 `koa-ejs` 的基本能力
