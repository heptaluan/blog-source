...
这是因为 `promise` 是基于任务队列机制的（详细可以参考 [JavaScript 并发模型](http://localhost:4000/2018/08/12/JavaScript/30/)），即当前代码执行完的时候才会触发，但是会在下一个 `Event Loop` 之前执行（注意与 `setTimeout` 区分开来）
