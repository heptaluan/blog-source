...
这是因为 `promise` 是基于任务队列机制（`Microtasks`）的，即当前代码执行完的时候才会触发，但是会在下一个 `Event Loop` 之前执行（注意与 `setTimeout` 区分开来）
...
上面我们已经完成了一个基本版的启动器函数，支持 `yield` 后跟回调函数或者 `Promise` 对象

但是并不完善，比如我们没有针对 `Generator` 进行错误捕获，所以我们可以考虑将其封装成一个 `Promise` 的形式
...
核心代码入口是 `onFulfilled`，无论如何第一次的 `next(ret)` 是一定要执行的，因为 `generator` 必须要 `next()` 一下的

但是 `co` 实际上有两种调用方式，分为有参数和无参数的，很明显以上是无参数的 `generator` 执行器，那么有参数的 `wrap` 呢？`co` 为我们提供了简单的包装
