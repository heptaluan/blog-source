...
在之前我们已经了解过 `Node.js` 和 `koa.js` 的一些相关知识，今天我们来看看如何使用 `egg.js` 来实现一个接口服务，`egg.js` 是一个基于 `koa.js` 的框架实现的框架，所以它应当属于框架之上的框架，它继承了 `koa.js` 的高性能优点，同时又加入了一些 **约束与开发规范**，来规避 `koa.js` 框架本身的开发自由度太高的问题
