...
之前我们介绍过了 [单链表](http://localhost:4000/2020/02/01/Essay/07/)，对于单链表，由于每个结点只存储了向后的指针，到了尾部标识就停止了向后链的操作（也就是 `NULL`，空指针）
...
下面我们就来看看 **单循环链表** 的代码如何实现，其实本质上和我们在 [单链表](http://localhost:4000/2020/02/01/Essay/07/) 一节当中实现的单链表差异不大
