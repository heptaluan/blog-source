...
在之前的章节当中，我们介绍过了 [单链表](http://localhost:4000/2020/01/12/Essay/07/) 和 [循环链表](http://localhost:4000/2020/02/06/Essay/09/) 相关内容，今天我们就来看看它们的升级版本，也就是所谓的双向链表与双向循环链表
...
双向链表提供了两种迭代列表的方法，从头到尾，或者反过来从尾到头，在双向链表当中我们也可以访问一个特定节点的下一个或前一个元素，因为如果在单向链表中，如果迭代列表时错过了要找的元素，就需要回到列表起点，重新开始迭代，这也是双向链表的一个优点
