...
之前我们介绍过了 [单链表](https://heptaluan.github.io/2020/02/01/Essay/07/) 和 [循环链表](https://heptaluan.github.io/2020/02/04/Essay/09/)，今天我们就来看看双向链表与双向循环链表
...
比如某个城市的地铁是一个环形的，就类似于我们的循环链表

```console
A -> B -> C -> D -> E -> A
```

现在某人从 `A` 站上车，最后到达了 `E` 站，但是他发现坐过站了，他其实要去的是 `D` 站，那么按照我们的循环链表的约定，他要经过的路线是
...
双向链表的 `insert` 方法与单链表相似，但需要设置新节点的 `previous` 属性，使其指向该节点的前驱，定义如下（`find()` 方法的实现是一致的）
