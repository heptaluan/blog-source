...
在之前的章节当中我们介绍过了 [栈](http://localhost:4000/2020/02/22/Essay/11/) 的相关知识，本篇我们就接着上篇未完的内容来了解一下和栈十分相似的队列的相关知识
...
队列（`queue`）是 **只允许在一端进行插入操作，而在另一端进行删除操作** 的 **线性表**，与栈相反，队列是一种先进先出（`First In First Out`，`FIFO`）的线性表，与栈相同的是，队列也是一种重要的线性结构，实现一个队列同样需要顺序表或链表作为基础，如下图所示
...
循环队列它的容量是固定的，并且它的队头和队尾指针都可以随着元素入出队列而发生改变，这样循环队列逻辑上就好像是一个环形存储空间，但要需要注意的是，在实际的内存当中，不可能有真正的环形存储区，我们只是用顺序表模拟出来的逻辑上的循环，可以通过下图来进行了解
