...
在上面我们实现了对于已经创建好的组件的动态加载，那么如果是还未创建的组件呢？其实简单来说这种情况就是我们需要 **动态创建不存在的组件** 而不是已经声明的组件，不过这种情况一般很少遇见，但是如果遇到可以考虑使用 `Compiler`，它作用就是用于在运行的时候运行 `Angular` 编译器来创建 `ComponentFactory` 的服务，然后可以使用它来创建和呈现组件实例
