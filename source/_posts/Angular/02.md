...
一个简单的 `Person` 类，依赖 `Id` 和 `Address` 两个类
...
完成以后可以发现，相对于之前的示例，依赖性已经不再是那么强烈

但是这里还是存在一个小的问题，那就是在 `main()` 方法中依然需要知道 `Id` 和 `Address` 两个类的具体构成
...
export class AppComponent {
  ...
  constructor() {
    ...
    const injector = ReflectiveInjector.resolveAndCreate([
      ...
      // 针对于 { provide: Person, useClass: Person }, 这种 useClass 可以直接简写为 Person
