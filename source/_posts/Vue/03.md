...
下面我们就来看看其中的一些 `API` 的用法，比如之前介绍过的 `State`，`Getter`，`Mutation` 和 `Action` 等
...
这也就是为什么我们在 `Vue` 的组件中可以通过 `this.$store.xxx` 来访问到 `Vuex` 的各种数据和状态的原因了，因为在任意组件中执行 `this.$store` 都能找到装载的那个 `store` 对象
