...
在上一章 [Vuex 框架核心流程](https://heptaluan.github.io/2018/08/13/Vue/07/) 当中，我们介绍了 `Vuex` 的一些常见的 `API` 以及初始化装载与注入的流程
...
通过观察可以发现，其实 `Vuex` 真正核心的就是 `Store` 这个东西，而始化装载与注入（`install`）我们在上一章已经介绍过了，剩下的一些也都是一些辅助方法

所以在本章，我们就深入的来了解一下这个 `Store` 到底是什么东西

通常我们在使用 `Vuex` 的时候，会实例化 `Store` 类，然后传入一个对象

包括我们定义好的 `actions`，`getters`，`mutations`，`state` 等，甚至当我们有多个子模块的时候，我们可以添加一个 `modules` 对象
