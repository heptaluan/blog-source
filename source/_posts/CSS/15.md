...
某些可替换元素，例如 `<iframe>` 元素，可能具有自己的样式表，但它们不会继承父文档的样式，`CSS` 能对可替换元素产生的唯一影响在于，部分属性支持控制元素内容在其框中的位置或定位方式
...
```html
<div>
  <img src="https://gitee.com/heptaluan/backups/raw/master/cdn/cover/15.jpg">
</div>
```
...
原理是当设置 `text-align` 的时候，内联元素文字和图片会居中显示，我们让空白节点的行高与 `div` 高度一致，这样就可以实现垂直居中，图片和空白节点默认基线对齐

另外我们也可以将其中的图片替换为多行文本从而实现多行文本垂直居中，代码如下
