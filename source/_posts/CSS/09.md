...
本文主要参考 [CSS 伸缩盒布局模组](https://www.w3.org/html/ig/zh/css-flex-1/)

主要分为两部分，容器和元素，这里主要说的是应用在元素身上的 `flex` 属性取值的问题

下面是一些 `flex` 布局常用的属性和一些取值介绍
...
![](https://gitee.com/heptaluan/backups/raw/master/cdn/css/14.png)
...
当 `item-1` 基准值取 `0%` 的时候，是把该项目视为零尺寸的，故即便声明其尺寸为 `9999px`，也并没有什么用，形同虚设

而 `item-2` 基准值取 `auto` 的时候，根据规则基准值使用值是主尺寸值即 `100px`，故这 `100px` 是会用来计算基准值，而不会纳入剩余空间
