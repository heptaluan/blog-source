...
在平常开发过程当中，虽然 `Object` 上的一些原生方法经常会看到，比如在一些扩展的第三方插件中可能会遇到，但是遇到了类似于 `Object.fromEntries()` 和 `Object.entries()` 这种长的比较像的但是使用频率较低的 `API`，往往又是傻傻分不清楚，更别说它们具体是做什么用的
...
`Object` 上面涉及到的方法其实不算很多，大致都可以按类别划分，所以我们将会分类来进行介绍，下表是一些本文没有涉及到的方法，一些在之前的文章当中我们也都已经介绍过了，感兴趣的话可以自行参考，所以这里就不详细展开了
...
| `Object.is()` | 判断两个值是否是 [相同的值](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Equality_comparisons_and_sameness)（同 `===`），但是这个可以比对 `NaN` |
