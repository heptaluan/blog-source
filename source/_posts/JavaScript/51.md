...
另外一个例子就是将 `Object` 转换为 `Map`，`new Map()` 函数接受一个可迭代的 `entries`，借助 `Object.entries` 方法可以很容易的将 `Object` 转换为 `Map`
...
而 `Object.getPrototypeOf(Object)` 的意思是把 `Object` 这一构造函数看作对象，返回的当然是函数对象的原型，也就是 `Function.prototype`，所以结果是 `true`

所以正确的方法应该是，`Object.prototype` 是构造出来的对象的原型
...
| `Object.prototype.isPrototypeOf()` | 用于测试一个对象是否存在于另一个对象的原型链上，见 [JavaScript 中的类型判断](http://localhost:4000/2018/03/17/JavaScript/25/#Object-prototype-isPrototypeOf) |
