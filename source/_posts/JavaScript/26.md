...
关于 `promise` 到底是什么东西，我们从为什么要使用它来看起
...
首先，我们要知道我们为什么要使用 `promise`，先来看下面这个示例，读取一个文件，在控制台输出这个文件内容
...
这就是传说中的 `callback hell`，其实也不止单单的嵌套和缩进的问题，比如在之中存在大量的逻辑判断等，这这种情况下，我们就可以考虑使用 `promise`
...
基本的概念就是这些，如果想要了解更多，可以参考阮一峰老师的 [《ECMAScript 6 入门》](http://es6.ruanyifeng.com/#docs/promise)
...
一个经典的题目，黄灯一秒亮一次，绿灯两秒亮一次，红灯三秒亮一次，如何让三个灯不断交替重复亮灯？
