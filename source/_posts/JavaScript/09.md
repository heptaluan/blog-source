...
`null` 表示没有对象，即该处不应该有值，所以 `Object.prototype.__proto__` 的值为 `null` 跟 `Object.prototype` 没有原型，其实表达了一个意思，所以查找属性的时候查到 `Object.prototype` 就可以停止查找了，这也就是开头部分的图片当中所表达的这个意思
...
笼统的说，都是指向了构造函数，但实际上是指向了实例化的对象，我们可以通过模拟 `new` 操作的过程来进行了解，所以首先我们要知道 `new` 做了什么
