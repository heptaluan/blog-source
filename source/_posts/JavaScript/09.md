...
但是当我们删除了 `f1` 的 `name` 属性时，再次读取 `f1.name` 的时候，从 `f1` 对象中找不到 `name` 属性就会从 `f1` 的原型也就是 `f1.__proto__`

对应的也就是 `Foo.prototype` 中查找，幸运的是我们找到了 `name` 属性，结果为 `zhangsan`
