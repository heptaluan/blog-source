...
有一些是我们经常会遇到和用到的，比如 `let`，`const`，扩展运算，`promise` 等，之前也单独整理过一些，比如 [Class](http://localhost:4000/2017/09/20/JavaScript/19/)，[Iterator](http://localhost:4000/2019/07/13/JavaScript/44/)，[Async，Await](http://localhost:4000/2020/01/16/JavaScript/50/) 和 [Reflect](http://localhost:4000/2017/09/21/JavaScript/43/) 等
...
`ES6` 引入了一种新的原始数据类型 `Symbol`，表示独一无二的值，比如如下代码，变量 `s` 就是一个独一无二的值，通过 `Symbol` 函数生成，使用 `typeof` 结果为 `'symbol'`
...
`Symbol` 函数可以接受一个字符串作为参数，表示对 `Symbol` 实例的描述，主要是为了在控制台显示，或者转为字符串时比较容易区分，比如如下示例，如果不加参数，它们在控制台的输出都是 `Symbol()`，不利于区分
...
`Symbol.for()` 接受一个字符串作为参数，然后搜索有没有以该参数作为名称的 `Symbol` 值，如果有就返回这个 `Symbol` 值，否则就新建一个以该字符串为名称的 `Symbol` 值，**并将其注册到全局**，也就是说尽管是在函数内部运行的，但是生成的 `Symbol` 值是登记在全局环境的
...
注意与 `Symbol('foo')` 区分开来，`Symbol('foo')` 每次调用都会返回不同的 `Symbol` 值，而 `Symbol.keyFor()` 则是用于返回已登记的 `Symbol` 类型值的 `key`，但是需要注意，只能返回 `Symbol.for()` 的 `key`
