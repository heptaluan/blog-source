...
Array.prototype.map = function (callbackFn, thisArg) {
  if (this === null || this === undefined) {                                // 处理数组类型异常
    throw new TypeError(`Cannot read property 'map' of null or undefined`)
  }
  if (Object.prototype.toString.call(callbackfn) != '[object Function]') {  // 处理回调类型异常
  ...
  let A = new Array(len)
  for (let k = 0; k < len; k++) {
    if (k in O) {                                                           // 使用 in 来进行原型链查找，同时如果没有找到就不处理，能有效处理稀疏数组的情况
      let kValue = O[k]
