...
关于防抖和节流，是一个老生常提的话题了，随便在网上搜一搜，都可以找到现成可以使用的函数，但是现成的函数的功能实现的都是已经比较完善的了，我们并不清楚它如何或者为什么要这样设计，所以我们今天就从头开始的来深入的了解一下这两个方法
...
所以针对这种情况，我们添加一个 `immediate` 参数判断是否是立刻执行，如果 `immediate` 参数传递为 `true`，则每隔 `wait` 秒后才会再次执行
