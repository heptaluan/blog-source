...
我们在之前的章节当中梳理了 [浏览器缓存机制](http://localhost:4000/2019/11/12/HTTP/04/) 相关内容，乘热打铁，我们本章当中来看看浏览器存储机制的相关内容，主要涉及到 `Cookie`，`localStorage`，`sessionStorage`，`Web SQL` 和 `IndexedDB` 相关内容，下面我们就一个一个来进行介绍
