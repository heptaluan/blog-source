...
* `options`，字典类型的参数，接受以下字段
  ...
  * `cancelable`，可选，`Boolean` 类型，默认值为 `false`，表示该事件能否被取消，如果为 `true`，那么默认行为就会被阻止
...
但是这种方式存在一定的局限性，那就是无法传递参数，如果我们想要传递参数的话，则可以使用 `CustomEvent`，从技术上讲 `CustomEvent` 和 `Event` 一样，除了一点不同，即在第二个参数（对象）中，我们可以为我们想要与事件一起传递的任何自定义信息添加一个附加的属性 `detail`，下面是一个简单的示例
