...
包括内置对象函数在内的所有函数都可以用 `new` 来调用，这种函数调用被称为构造函数调用，实际上并不存在所谓的构造函数，只有对于函数的构造调用，使用 `new` 来调用函数，会自动执行以下操作
...
这里多介绍一些，我们从上面可以得知，在 `new` 调用的过程当中会依次执行一些操作，所以我们可以使用一个函数来模拟这种行为，也是就是模拟 `new` 的实现，如下
...
在上面的例子中，我们使用构造函数生成了一个对象 `p1`，该对象同时具有 `moveTo` 方法，然后使用对象字面量创建了另一个对象 `p2`，我们看到使用 `apply` 可以将 `p1` 的方法应用到 `p2` 上
...
更为详细的内容可以参考 [规范 15.3.4.4 - Function.prototype.call()](http://lzw.me/pages/ecmascript/#323)
...
当函数被用作事件处理函数时，它的 `this` 指向触发事件的元素，需要注意，`IE` 的 `attachEvent()` 中的 `this` 指向 `window`（`IE11+` 已经支持 `addEventListener()`）
...
规范指出，当执行流进入函数代码时，由函数调用者提供 `thisArg` 和 `argumentsList`

在 [11.2.3 函数调用](http://yanhaijing.com/es5/#164) 当中我们可以发现，在函数调用发生时，首先会对 **函数名部分进行计算** 并赋值给 `ref`，并且通过一系列的判断就可以来决定 `this` 会指向何方，如下图所示
