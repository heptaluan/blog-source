...
我们从上面可以得知，在 `new` 调用的过程当中会依次执行一些操作，所以我们可以使用一个函数来模拟这种行为，也是就是模拟 `new` 的实现，如下
...
在上面的例子中，我们使用构造函数生成了一个对象 `p1`，该对象同时具有 `moveTo` 方法，然后使用对象字面量创建了另一个对象 `p2`，需要注意此时的 `p2` 是没有 `moveTo` 这个方法的，但是我们可以使用 `apply` 来将 `p1` 的方法应用到 `p2` 上
...
所以今天我们就借助 [ECMAScript 规范](http://yanhaijing.com/es5/#155) 来深入的来了解一下，到底 `this` 是个什么东西，它是如何来进行定义的
...
第一个我们就慢慢来看，根据上面的流程可知，我们首先需要做的工作就是计算 `MemberExpression` 的结果

通过计算可以得出，第一个示例的 `MemberExpression` 的结果是 `foo.bar`，即是通过属性表达式来进行访问的，而我们在之前的左值表达式当中已经介绍过了，该表达式返回了一个 `Reference` 类型，所以我们第一步的工作已经完成了，即确定了它是 `Reference` 类型

有了 `ref` 以后，所以接下来我们就可以先来进行第一次的判断，即判断 `IsPropertyReference(ref)` 的值
...
到了这一步，就不用继续往下走了，我们已经可以断定 `this` 的值了，也就是取 `GetBase(ref)`，而 `GetBase(ref)` 就是返回 `reference` 的 `base value`，所以 `this` 的值就是 `foo`
