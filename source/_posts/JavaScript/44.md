...
在了解了上面的例子以后，我们就会考虑，那么这样一来我们的迭代器对象是不是就可以进行遍历了，我们来试一下
...
简单来说，只要**一种数据结构部署了 Iterator 接口，我们就称这种数据结构是可遍历的（iterable）**
...
如果一个对象要具备可被 `for...of` 循环调用的 `Iterator` 接口，就必须在 `Symbol.iterator` 的属性上部署遍历器生成方法，比如一个实现 `50` 以内的斐波纳契数列的示例
