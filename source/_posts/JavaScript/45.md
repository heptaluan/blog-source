...
详细可见 [函数防抖和节流](https://heptaluan.github.io/2020/01/05/JavaScript/49/)
...
可以使用 [尾调用](http://es6.ruanyifeng.com/#docs/function#%E5%B0%BE%E8%B0%83%E7%94%A8%E4%BC%98%E5%8C%96)（某个函数的最后一步是调用另一个函数）来进行优化

我们需要做的就是把所有用到的内部变量改写成函数的参数
...
这里关于乱序需要说明一下，因为这里涉及到了 `sort()` 方法，所以上面的结果应该是有问题的，不信我们来测试一下

比如下面这里例子，将 `[1, 2, 3, 4, 5]` 乱序 `10` 万次，计算乱序后的数组的最后一个元素是 `1、2、3、4、5` 的次数分别是多少
...
根据结果可以发现，这样的算法的确是有问题的，具体原因可以参考 [JavaScript专题之解读 v8 排序源码](https://juejin.im/post/59e80dc6f265da432a7aaf15)

这里我们只考虑如何解决这样的问题，针对于乱序的问题，我们可以采用 [Fisher–Yates_shuffle](https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle)

比如如下两种方式，都是采用的「摸牌」方式
