...
一些手写 `API`，常用方法的实现，常见算法的汇总，在这里做下记录，方便以后可以快速查询（可以直接参考左边目录）
...
可以使用 [尾调用](http://es6.ruanyifeng.com/#docs/function#%E5%B0%BE%E8%B0%83%E7%94%A8%E4%BC%98%E5%8C%96)（某个函数的最后一步是调用另一个函数）来进行优化，我们需要做的就是把所有用到的内部变量改写成函数的参数
...
第一种，也是最原始的方法，即双层循环（`while` 也可），其缺点是其复杂度为 `O(n ^ 2)`，如果数组过大，将会影响性能，但是它也有好处，就是兼容性好
