...
要想改变被调用函数的上下文，可以使用 [call 或 apply 方法](https://heptaluan.github.io/2018/04/16/JavaScript/32/)，但如果重复使用就会很不方便，因为每次都要把上下文对象作为参数传递，而且还会使代码变得不直观
...
`bind` 方法的定义见 [Function.prototype.bind()](http://lzw.me/pages/ecmascript/#324)
...
这个特性可以用于创建一个函数的捷径，之后我们可以通过这个捷径调用，以便在代码多处调用时省略重复输入相同的参数，也就是下面会说到的 `currying`
...
`add` 函数的作用是把参数 `a, b, c` 进行拼接（或者说相加），但是有的时候不需要一次把这个函数都调用完成

而是调用一次把前两个参数传完了以后，然后得到了这样的一个函数，再去调用，并且每次传入第三个值
...
Function.prototype.bind = function (oThis, ...args) {
  ...
  let fToBind = function (...params) {
    ...
    return context.apply(context, ...args, ...params)
  }
  fToBind.prototype = Object.create(context.prototype)
  return fToBind
}
```
