...
对于这两个方法，在以前的工作当作也仅仅只是使用，但是并没有去深入研究，只是简单的知道一个是将对象或者数组转为 `JSON` 字符串，另外一个就是反过来转换 `JSON` 的
...
方法是将一个 `JavaScript` 值（对象或者数组）转换为一个 `JSON` 字符串，如果指定了 `replacer` 是一个函数，则可以替换值，或者如果指定了 `replacer` 是一个数组，可选的仅包括指定的属性，基本语法如下
