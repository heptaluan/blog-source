...
我们在之前的 [React 中的 Hook](https://heptaluan.github.io/2020/10/31/React/16/) 章节当中已经梳理过了 `Hook` 的一些相关内容，了解了 `Hook` 的几种使用方式以及它内部一些简单的运行原理，所以在本章当中我们来继续深入了解一些自定义 `Hook` 的相关内容，主要原因是因为最近的项目当中使用了较多的 `Hook`，也踩了不少的坑，所以也算是在这里简单的总结梳理一下
...
这个不用介绍太多，正如其名，我们可以使用它来获取元素 `resize` 后的长宽
...
这个也是我们平常使用较多的一个自定义 `Hook`，所以我们就来详细的梳理一下它的实现流程，简单来说，不管我们选择哪种方式来获取数据，`Axios`、`Fetch API`，还是其它，我们很有可能在 `React` 组件当中一次又一次地编写相同的代码，因此我们来看看如何构建一个简单但有用的自定义 `Hook`，以便在需要在应用程序内部获取数据时调用该 `Hook`，我们就称其为 `useFetch`

这个 `Hook` 接受两个参数，一个是获取数据所需查询的 `URL`，另一个是表示要应用于请求的选项的对象，它的大致轮廓如下
...
由于获取数据是一个副作用，所以我们应该在 `useEffect` 当中来进行执行，我们会传递 `URL` 和 `options`，而返回的 `Promise` 我们则使用 `json()` 方法处理后将它存储在一个 `state` 变量中即可
...
下面我们再来看看返回值，我们的 `useFetch` 应该返回一个对象，其中包含从 `URL` 中获取的数据，并且如果发生了任何错误，则应该返回错误
...
下面我们再来看看如何进行使用，我们只需要传递我们想要检索的资源的 `URL`，然后我们可以得到一个对象，这样我们就可以使用得到的数据来渲染我们的应用程序
...
最后我们再来看看如何使用，以下面这个组件为例，检测 `DOM` 元素外部的点击，如果用户单击对话框组件，则在此处关闭对话框组件
...
然后返回一个数组，类似于使用 `useState` 获得的数组，因此此数组将包含有状态值和在将其持久存储在 `localStorage` 中时对其进行更新的函数，下面我们先来创建将与 `localStorage` 同步的 `React` 状态变量
