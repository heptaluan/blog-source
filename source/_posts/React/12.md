...
在上面我们虽然手动实现了 `Redux` 的整体流程，但是需要注意的是，现在的实现跟 `React.js` 一点关系都没有，但是接下来我们会把 `React.js` 和 `Redux` 结合起来，用 `Redux` 模式帮助我们来管理 `React.js` 的应用状态，其实也就是 `React-Redux` 的实现，关于这部分内容，我们会另起篇幅来进行介绍
