...
不过与 `Class` 组件中的 `setState` 方法不同，`useState` 不会自动合并更新对象，你可以用函数式的 `setState` 结合展开运算符来达到合并更新对象的效果
...
另外比较重要的一点就是，如果函数组件被 `React.memo` 包裹，并且其实现中拥有 `useState` 或 `useContext` 的 `Hook` 的时候，那么当 `context` 发生变化时，它是仍会重新渲染的，所以在使用的时候一定要小心
...
export default class TwentyOne extends React.Component {
  render() {
    return (<div>
      <TwentyOneChild name={`React.PureComponent`}></TwentyOneChild>
    </div>)
  }
}
```
