...
我们都知道，React 是一个用于构建用户交互界面的 JavaScript 库，其核心机制就是跟踪组件的状态变化，并将更新的状态映射到到新的界面。在 React 中，我们将此过程称之为协调。我们调用 setState方法来改变状态，而框架本身会去检查 state 或 props 是否已经更改来决定是否重新渲染组件。React 的官方文档对 [协调机制](https://reactjs.org/docs/reconciliation.html) 进行了良好的抽象描述，即
...
之所以会推出新的 Fiber，那就说明在 React 15 当中是存在一定的问题的，那么下面我们就来看看之前到底是存在哪些问题，这个我们可以从官方提供的 [示例](https://claudiopro.github.io/react-fiber-vs-stack-demo/) 开始看起，运行效果如下

![](https://gitee.com/heptaluan/backups/raw/master/cdn/react/18-01.gif)

观察上面的图片我们可以发现，运行起来感觉十分的卡顿，其根本原因，是因为大量的同步计算任务阻塞了浏览器的 UI 渲染。默认情况下，JavaSceipr 运算、页面布局和页面绘制都是运行在浏览器的主线程当中，他们之间是互斥的关系。如果 JS 运算持续占用主线程，页面就没法得到及时的更新。当我们调用setState更新页面的时候，React 会遍历应用的所有节点，计算出差异，然后再更新 UI。整个过程是一气呵成，不能被打断的。如果页面元素很多，整个过程占用的时机就可能超过 16 毫秒（至于为什么是 16 毫秒我们下面会进行介绍），就容易出现掉帧的现象。

所以针对这一问题，React 团队就从框架层面对 Web 页面的运行机制做了优化，从而得到了很好的效果，也就如下图所示

![](https://gitee.com/heptaluan/backups/raw/master/cdn/react/18-02.gif)

我们从视觉上可以明显的感觉到，第二个示例运行起来会明显感觉比第一个示例看上去流畅许多，那么你有没有考虑过这样一个问题，那就是为什么会卡顿？


## 为什么会卡顿？

我们可以查看上面示例的代码来分析分析原因，源码可见 [react-fiber-vs-stack-demo](https://github.com/claudiopro/react-fiber-vs-stack-demo)，通过观察我们可以发现，代码使用了 requestAnimationFrame（这里我们简称为 rAF）
...
这里关于 requestAnimationFrame 这个接口的使用我们就不涉及更多了，详细可以参考之前我们整理过的 [requestAnimationFrame 的使用](http://localhost:4000/2018/06/11/JavaScript/38/#window-requestAnimationFrame) 以及 [并发模型中的 requestAnimationFrame](http://localhost:4000/2018/08/12/JavaScript/30/#%E5%85%B3%E4%BA%8E-requestAnimationFrame)

这里我们简单总结一下就是，对绝大多的浏览器来说，它的页面刷新频率取决于显示器的刷新频率。以我的显示器为例，它的刷新频率是 60fps，rAF 会每 16.66ms（1000ms / 60）执行一次回调函数。但为了给主线程时间处理其他事务，一般要求产出一帧画面的时间不要超过 10ms 。rAF 的执行频率不会比显示器更高，但有可能更低
...
function render(timeStamp){
    ...
    lastTime = timeStamp;
    console.log("%s fps",1000/period);
    var time = Date.now();
    while(Date.now() - time < 333){} // 强行占用主线程 333ms
    requestAnimationFrame(render);
}
requestAnimationFrame(render);
```

在你的浏览器 console 中运行这段代码，你会看到 rAF 回调的执行频率只有 3fps。因为浏览器的 javascript 引擎是单线程的，因此在执行 while 期间主线程就无法抽身做其他事情。此外如果此时与浏览器进行互动（比如点个关注、赞同什么的），你会发现动画反应有些迟钝。这是因为除了服务 js 引擎，响应用户事件和绘制界面也是在主线程上运行的，while 把他们也阻塞了。刷新页面，把这段代码中的 while 注释掉再执行一遍，看到频率又回到了正常水平。

回头看 setInterval 就没什么好说的了。它和 rAF 一样执行间隔不会比 1 秒更短，但有可能更长，要看主线程当前有没有空为它服务。

显然在 Stack Reconciliation 的例子中主线程被其他事务耗尽了，rAF 和 setInterval 回调的执行遭到了延迟，所以画面出现了卡顿。现在让我们来看看是谁把主线程榨的一滴也没有了。

要解决这个问题就得从两方面入手：

* 减少 js 的执行量。像 update 回调，虽然每次只更新了顶层 div 的 style，但它依旧 re-render 到了最细枝末节的 Dot 组件。这里 99%的工作都是在浪费资源；
* 如果无法避免大量执行 js，就想办法寻找合适的时机执行

下面我们就来看看Fiber Reconciliation 的策略
