...
上述只是一个简单的示例，只能适用于普通的标签，但并不适用于 **组件**，为了能够渲染组件，我们还需要思考组件的 `VNode` 应该如何表示？对于 `html` 标签的 `VNode` 来说，其 `tag` 属性的值就是标签的名字，但 **如果是组件的话，我们可以将其 VNode 中的 tag 属性指向组件自身**，比如如下组件
