...
这是因为 `Context` 是一个危险的特性，所以 `React` 团队将其使用方式设置的复杂一些，提高使用门槛的同时也会让你注意到它的危险性，如果你要给组件设置 `Context`，那么 `childContextTypes` 是必写的
...
export default class ThemeSwitch extends Component {
  ...
  render() {
    return (<div style={{ color: this.state.themeColor }}>
      <button>Red</button>
      <button>Blue</button>
    </div>)
  }
