...
之前在学习 `koa.js` 当中的 [中间件](https://heptaluan.github.io/2019/10/19/Node/12/) 部分时，我们接触到过中间件这个概念，也了解洋葱模型这个概念，所以在今天我们就来深入的了解一下 `redux` 当中与其十分类似的 `middleware`
...
`middleware` 的设计有点特殊，是一个层层包裹的匿名函数，这其实是函数式编程中的 [柯里化](https://zh.wikipedia.org/zh/%E6%9F%AF%E9%87%8C%E5%8C%96)，`applyMiddleware` 会对 `logger` 这个 `middleware` 进行层层调用，动态地对 `store` 和 `next` 参数赋值
...
在之前我们已经介绍过，通过匿名函数的方式 `middleware` 中拿到的 `dispatch` 和最终 `compose` 结束后的新 `dispatch` 是保持一致的，所以在 `middleware` 中调用 `store.dispatch()` 和在其他任何地方调用效果是一样的，而在 `middleware` 中调用 `next()` 的效果则是进入下一个 `middleware`
