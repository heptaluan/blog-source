...
原来的生命周期在 `React 16` 推出的 `Fiber` 之后就不合适了（关于 `React Fiber` 相关内容可以见 [深入 React Fiber](http://localhost:4000/2020/12/06/React/18/)），因为如果要开启 `async rendering`，那么在 `render` 函数之前的所有函数，都有可能执行多次，也就是
