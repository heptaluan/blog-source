...
也就是上述代码中类的构造方法（`constructor`），`Test` 类继承了 `React Component` 这个基类，也是因为继承了这个基类，才能拥有 `render()`，生命周期等方法可以使用，这也说明了为什么函数组件不能使用这些方法的原因
...
弄清楚了 `React` 组件的更新机制，我们回归正题，继续梳理更新阶段，此阶段分为下面几个步骤
...
在 `React 16.4` 版本中，让 `getDerivedStateFromProps` 无论是挂载（`mounting`）还是更新（`updating`），又或是其他什么引起的更新，全部都会被调用
...
`getSnapshotBeforeUpdate()` 被调用于 `render` 之后，可以读取但无法使用 `DOM` 的时候，它可以让我们的组件在可能更改之前从 `DOM` 捕获一些信息（例如滚动位置），此生命周期返回的任何值都将作为参数传递给 `componentDidUpdate`
