...
`jsx` 本质上是 `React.createElement(component, props, ...children)` 方法提供的语法糖
...
export default class AutoFocusTextInput extends Component {
  ...
  render() {
    return (
      ...
      // 当 input 元素在页面上挂载完成以后，React 就会调用这个函数，并且把这个挂载以后的 DOM 节点传给这个函数
      // 在函数中我们把这个 DOM 元素设置为组件实例的一个属性，这样以后我们就可以通过 this.input 获取到这个 DOM 元素

      // 但是记住一个原则：能不用 ref 就不用
      // 特别是要避免用 ref 来做 React 本来就可以帮助你做到的页面自动更新的操作和事件监听
      <div>
...
`React` 还提供了一个组件，用于在没有外层父元素包裹的情况下渲染多个元素
...
还可以使用一种更简短的语法来声明 `Fragments`，也可以理解为简写方式
