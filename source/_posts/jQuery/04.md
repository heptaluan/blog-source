...
这个方法主要用于判断参数的 `JavaScript` 类型，在平常的开发当中也是经常会遇到的，如果参数是 `undefined` 或 `null`，返回 `'undefined'` 或 `'null'`（注意是字符串类型）
...
用于查找数组当中满足过滤函数的元素，原数组不会受影响，如果参数 `invert` 没有传入或者为 `false` 元素只有在过滤函数返回 `true`
