...
之前我们在 [jQuery 源码的总体架构](http://localhost:4000/2017/06/23/jQuery/02/) 当中介绍了 `jQuery` 的总体架构，和构造函数 `jQuery` 的一些常见用法

本章我们就来深入的了解一下初始化方法 `return new jQuery.fn.init(selector, context, rootjQuery)`，源码如下
...
init: function(selector, context, rootjQuery) {
  ...
  // ② 参数 selector 可以转换为 false，例如是 undefined、空字符串、null 等则直接返回 this，此时 this 是空 jQuery 对象，其属性 length 等于 0
  // 如果 selector 为空，!selector 为 false，比如 $(''), $(null), or $(undefined)
  if (!selector) {
    // 此时 this 为空 jQuery 对象
    return this
  }

  // ③ 如果参数 selector 有属性 nodeType，则认为 selector 是 DOM 元素，比如 $(DOMElement)
  ...
  // ④ 如果参数 selector 是字符串 'body'，手动设置属性 context 指向 document 对象，第一个元素指向 body 元素，最后返回包含了 body 元素引用的 jQuery 对象
